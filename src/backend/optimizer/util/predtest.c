...
arrayconst_startup_fn(Node *clause, PredIterInfo info) {
    ...
    constexpr
    .
    consttype = ARR_ELEMTYPE(arrayval);
    state->
    constexpr
    .
    consttypmod = -1;
    ...
    .
    constbyval = elmbyval;
    lsecond(state->opexpr.args) = &state->
    constexpr;
...
arrayconst_next_fn(PredIterInfo info) {
    ...
    constexpr
    .
    constvalue = state->elem_values[state->next_elem];
    state->
    constexpr
    .
    constisnull = state->elem_nulls[state->next_elem];
    state->next_elem++;
