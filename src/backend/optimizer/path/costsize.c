...
/* === helpers: 加权期望 & 期望页数（保持 simple/可内联） =================== */

static double
dist_mean(const Distribution *d, double fallback) {
    // CHANGE: 支持分布的加权期望；无分布时回退到单点
    if (!d || d->sample_count <= 0 || !d->probs || !d->vals)
        return fallback;

    double s = 0.0;
    for (int i = 0; i < d->sample_count; ++i)
        s += d->probs[i] * d->vals[i];
    return s;
}

static inline double
expect_pages(const Distribution *d, int width, double fallback_rows) {
    // CHANGE: 计算 E[ page_size(rows,width) ]；page_size 非线性需逐样本
    if (!d || d->sample_count <= 0 || !d->probs || !d->vals)
        return page_size(fallback_rows, width);
    double e = 0.0;
    for (int i = 0; i < d->sample_count; ++i)
        e += d->probs[i] * page_size(d->vals[i], width);
    return e;
}

/* === helpers: 仅用于把 cost_sort 改成对 Distribution 求期望 ================== */
// CHANGE: 对（可能有分布的）rows，逐样本调用 cost_sort，返回期望的 startup 与 delta(total-startup)
static inline void
expect_cost_sort(
    Path *base_path,
    PlannerInfo *root, List *sortkeys,
    Distribution *rows_dist, double fallback_rows,
    int width, double work_mem,
    Cost *E_startup, Cost *E_delta
) {
    if (!sortkeys) {
        *E_startup = 0;
        *E_delta = 0;
    ...
    /* 无分布/空分布：回退到原来的单点调用 */
    if (!rows_dist || rows_dist->sample_count <= 0 ||
        !rows_dist->probs || !rows_dist->vals) {
        Path sort_path;
        cost_sort(&sort_path, root, sortkeys,
                  base_path->total_cost, fallback_rows, width,
                  0.0, work_mem, -1.0);
        *E_startup = sort_path.startup_cost;
        *E_delta = sort_path.total_cost - sort_path.startup_cost;
        return;
    }
    ...
    for (int i = 0; i < rows_dist->sample_count; ++i) {
        Path sort_path;
        cost_sort(&sort_path, root, sortkeys,
                  base_path->total_cost, rows_dist->vals[i], width,
                  0.0, work_mem, -1.0);
        es += rows_dist->probs[i] * sort_path.startup_cost;
        ed += rows_dist->probs[i] * (sort_path.total_cost - sort_path.startup_cost);
    }
    *E_startup = es;
    *E_delta = ed;
}
...
cost_seqscan(Path *path, PlannerInfo *root,
             ...
             bool override, double rows) {
    elog(LOG, "cost_seqscan::[begin]");
    Cost startup_cost = 0;
    Cost cpu_run_cost;
    Cost disk_run_cost;
    ...
    if (override) {
        path->rows = rows;
        elog(LOG, "cost_seqscan::[override] rows = %.5f", path->rows);
    } else {
        if (param_info) {
            path->rows = param_info->ppi_rows;
            elog(LOG, "cost_seqscan::[use_ppi] rows = %.5f", path->rows);
        } else {
            path->rows = baserel->rows;
            elog(LOG, "cost_seqscan::[no_ppi] rows = %.5f", path->rows);
        }
    }
    ...
    elog(LOG, "cost_seqscan::[end] startup_cost = %.5f; total_cost = %.5f",
...
cost_samplescan(Path *path, PlannerInfo *root,
                RelOptInfo *baserel, ParamPathInfo *param_info,
                bool override, double rows) {
    elog(LOG, "cost_samplescan::[begin]");
    Cost startup_cost = 0;
    Cost run_cost = 0;
    RangeTblEntry *rte;
    ...
    if (override) {
        path->rows = rows;
        elog(LOG, "cost_samplescan::[override] rows = %.5f", path->rows);
    } else {
        if (param_info) {
            path->rows = param_info->ppi_rows;
            elog(LOG, "cost_samplescan::[use_ppi] rows = %.5f", path->rows);
        } else {
            path->rows = baserel->rows;
            elog(LOG, "cost_samplescan::[no_ppi] rows = %.5f", path->rows);
        }
    }
    ...
    elog(LOG, "cost_samplescan::[end] startup_cost = %.5f; total_cost = %.5f",
         path->startup_cost, path->total_cost);
}
...
cost_gather(
    ...
    /* Mark the path with the correct row estimate */
    if (rows) {
        elog(LOG, "cost_gather::[override]");
        path->path.rows = *rows; /* 保持原有 override 语义 */
    } else if (param_info) {
        elog(LOG, "cost_gather::[use_ppi]");
        // CHANGE: 若 PPI 提供分布，则用期望；否则回退 ppi_rows
        Distribution *ppi_dist = param_info->ppi_rows_dist; // CHANGE
        path->path.rows = dist_mean(ppi_dist, param_info->ppi_rows); // CHANGE
    } else {
        elog(LOG, "cost_gather::[no_ppi]");
        // CHANGE: 若 RelOptInfo 提供分布，则用期望；否则回退 rel->rows
        Distribution *rel_dist = rel->rows_dist; // CHANGE (可选)
        path->path.rows = dist_mean(rel_dist, rel->rows); // CHANGE
    }

    /* 子路径成本照旧：subpath 已经自身估好成本 */
    startup_cost = path->subpath->startup_cost;
    run_cost = path->subpath->total_cost - path->subpath->startup_cost;

    /* 并行的固定开销 + 每元组通信开销（线性于 rows，用期望即可） */
    startup_cost += parallel_setup_cost;
    run_cost += parallel_tuple_cost * path->path.rows; // 这里 rows 已是期望
...
cost_gather_merge(
    ...
    /* Mark the path with the correct row estimate */
    if (rows) {
        elog(LOG, "cost_gather_merge::[override]");
        path->path.rows = *rows; // 保持 override 行为
    } else if (param_info) {
        elog(LOG, "cost_gather_merge::[use_ppi]");
        // CHANGE: 若 PPI 有分布则取其期望，否则用 ppi_rows
        Distribution *ppi_dist = param_info->ppi_rows_dist; // CHANGE
        path->path.rows = dist_mean(ppi_dist, param_info->ppi_rows); // CHANGE
    } else {
        elog(LOG, "cost_gather_merge::[no_ppi]");
        // CHANGE: 若 RelOptInfo 有分布则取其期望，否则用 rel->rows
        Distribution *rel_dist = rel->rows_dist; // CHANGE
        path->path.rows = dist_mean(rel_dist, rel->rows); // CHANGE
    }
    ...
    /* Heap creation cost (与 rows 无关) */
    startup_cost += comparison_cost * N * logN;

    /* Per-tuple heap maintenance cost —— 线性于 rows：用期望 rows 即可 */
    run_cost += path->path.rows * comparison_cost * logN; // CHANGE: rows 已为期望

    /* small cost for heap management, like cost_merge_append —— 线性 */
    run_cost += cpu_operator_cost * path->path.rows; // CHANGE: rows 已为期望
     ...
     */
    startup_cost += parallel_setup_cost;
    run_cost += parallel_tuple_cost * path->path.rows * 1.05; // CHANGE: rows 已为期望
...
cost_index(IndexPath *path, PlannerInfo *root, double loop_count,
           bool partial_path, bool override, double rows) {
    elog(LOG, "cost_index::[begin]");
    ...
    elog(LOG, "cost_index::[quals] index_quals = %d, qpquals = %d",
         list_length(path->indexclauses), list_length(qpquals));
    ...
    elog(LOG, "cost_index::[end] startup_cost = %.5f; total_cost = %.5f",
...
cost_bitmap_heap_scan(Path *path, PlannerInfo *root, RelOptInfo *baserel,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_bitmap_heap_scan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_bitmap_heap_scan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_tidscan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_tidscan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_tidscan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_tidrangescan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_tidrangescan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_tidrangescan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_functionscan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_functionscan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_functionscan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_tablefuncscan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_tablefuncscan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_tablefuncscan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_valuesscan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_valuesscan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_valuesscan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_ctescan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_ctescan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_ctescan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_namedtuplestorescan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_namedtuplestorescan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_namedtuplestorescan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
cost_resultscan(Path *path, PlannerInfo *root,
    ...
    /* Mark the path with the correct row estimate */
    if (param_info) {
        elog(LOG, "cost_resultscan::[use_ppi]");
        path->rows = param_info->ppi_rows;
    } else {
        elog(LOG, "cost_resultscan::[no_ppi]");
        path->rows = baserel->rows;
    }
...
final_cost_nestloop(PlannerInfo *root, NestPath *path,
    ...
    if (path->jpath.path.param_info) {
        elog(LOG, "final_cost_nestloop::[use_ppi]");
        path->jpath.path.rows = path->jpath.path.param_info->ppi_rows;
    } else {
        elog(LOG, "final_cost_nestloop::[no_ppi]");
...
initial_cost_mergejoin_exp(
    ...
    /* CHANGE: 用分布期望替代单点行数（下界：Jensen 对 n log n） */
    Distribution *outer_dist = outer_path->rows_dist; // CHANGE
    Distribution *inner_dist = inner_path->rows_dist; // CHANGE
    double outer_path_rows = dist_mean(outer_dist, outer_path->rows); // CHANGE
    double inner_path_rows = dist_mean(inner_dist, inner_path->rows); // CHANGE
    ...
    /* Protect some assumptions below that rowcounts aren't zero (保留) */
    if (outer_path_rows <= 0) outer_path_rows = 1;
    if (inner_path_rows <= 0) inner_path_rows = 1;

    /* === 与选择率相关：保持原逻辑（这些不依赖行数分布） === */
    if (mergeclauses && jointype != JOIN_FULL) {
        RestrictInfo *firstclause = (RestrictInfo *) linitial(mergeclauses);
    ...
    /* Convert selectivities to row counts（用期望行数参与） */
    outer_skip_rows = rint(outer_path_rows * outerstartsel);
    inner_skip_rows = rint(inner_path_rows * innerstartsel);
    outer_rows = clamp_row_est(outer_path_rows * outerendsel);
    inner_rows = clamp_row_est(inner_path_rows * innerendsel);
    ...
    /* --- 外侧：如需排序，仅把 sort 成本改为“期望” ---------------------------- */
    if (outersortkeys) {
        /* do we need to sort outer? */
        // CHANGE: 期望版 cost_sort
        Cost E_sort_start = 0, E_sort_delta = 0;
        expect_cost_sort(
            outer_path, root, outersortkeys,
            outer_path->rows_dist, // CHANGE: 外侧分布
            outer_path_rows, // 回退用的行数
            outer_path->pathtarget->width,
            work_mem, &E_sort_start, &E_sort_delta
        );

        startup_cost += E_sort_start; // E[startup]
        startup_cost += E_sort_delta * outerstartsel; // E[delta]*开始段
        run_cost += E_sort_delta * (outerendsel - outerstartsel); // E[delta]*运行段
    } else {
        /* 原样保留 */
        startup_cost += outer_path->startup_cost;
        startup_cost += (outer_path->total_cost - outer_path->startup_cost)
    ...
    /* --- 内侧：如需排序，仅把 sort 成本改为“期望”；inner_run_cost 单独返回 --- */
    if (innersortkeys) /* do we need to sort inner? */
    {
        // CHANGE: 期望版 cost_sort
        Cost E_sort_start = 0, E_sort_delta = 0;
        expect_cost_sort(
            inner_path, root, innersortkeys,
            inner_path->rows_dist, // CHANGE: 内侧分布
            inner_path_rows, // 回退用的行数
            inner_path->pathtarget->width,
            work_mem, &E_sort_start, &E_sort_delta
        );

        startup_cost += E_sort_start; // E[startup]
        startup_cost += E_sort_delta * innerstartsel; // E[delta]*开始段
        inner_run_cost = E_sort_delta * (innerendsel - innerstartsel); // E[delta]*运行段
    } else {
        /* 原样保留 */
        startup_cost += inner_path->startup_cost;
        startup_cost += (inner_path->total_cost - inner_path->startup_cost)
    ...
    /* 这里不决定是否 rescan/materialize；只把内侧最小运行成本放进 total（保留原意） */
    ...
    workspace->run_cost = run_cost;
    workspace->inner_run_cost = inner_run_cost;
    workspace->outer_rows = outer_rows; // 已用期望行数推导
    workspace->inner_rows = inner_rows; // 已用期望行数推导
    workspace->outer_skip_rows = outer_skip_rows;
    workspace->inner_skip_rows = inner_skip_rows;
}
...
final_cost_mergejoin(PlannerInfo *root, MergePath *path,
    ...
    /* Mark the path with the correct row estimate */
    if (path->jpath.path.param_info) {
        elog(LOG, "final_cost_mergejoin::[use_ppi]");
        path->jpath.path.rows = path->jpath.path.param_info->ppi_rows;
    } else {
        elog(LOG, "final_cost_mergejoin::[no_ppi]");
        path->jpath.path.rows = path->jpath.path.parent->rows;
    }
 ...
 */
void
final_cost_mergejoin_exp(PlannerInfo *root, MergePath *path,
                         JoinCostWorkspace *workspace,
                         JoinPathExtraData *extra) {
    Path *outer_path = path->jpath.outerjoinpath;
    Path *inner_path = path->jpath.innerjoinpath;

    /* CHANGE: 用分布期望替代单点 inner_path->rows（无分布则回退） */
    Distribution *inner_dist = inner_path->rows_dist; // CHANGE
    double inner_path_rows = dist_mean(inner_dist, inner_path->rows); // CHANGE
    ...
    /* 这些来自 initial_cost_mergejoin，假定已是“代表值/期望值” */
    double outer_rows = workspace->outer_rows;
    double inner_rows = workspace->inner_rows;
    double outer_skip_rows = workspace->outer_skip_rows;
    double inner_skip_rows = workspace->inner_skip_rows;
    ...
    /* Protect some assumptions below that rowcounts aren't zero (保持原意) */
    if (inner_path_rows <= 0)
        inner_path_rows = 1;

    /* CHANGE: rows 标注也优先用分布（ppi 或 parent 的 rows_dist） */
    if (path->jpath.path.param_info) {
        elog(LOG, "final_cost_mergejoin::[use_ppi]");
        Distribution *ppi_dist = path->jpath.path.param_info->ppi_rows_dist; // CHANGE (可选字段)
        double ppi_rows = path->jpath.path.param_info->ppi_rows;
        path->jpath.path.rows = dist_mean(ppi_dist, ppi_rows); // CHANGE
    } else {
        elog(LOG, "final_cost_mergejoin::[no_ppi]");
        Distribution *parent_dist = path->jpath.path.parent->rows_dist; // CHANGE (可选字段)
        double parent_rows = path->jpath.path.parent->rows;
        path->jpath.path.rows = dist_mean(parent_dist, parent_rows); // CHANGE
    }

    /* For partial paths, scale row estimate. (原样) */
    if (path->jpath.path.parallel_workers > 0) {
        double parallel_divisor = get_parallel_divisor(&path->jpath.path);
        path->jpath.path.rows =
                clamp_row_est(path->jpath.path.rows / parallel_divisor);
    ...
    /* 评估 merge / qp quals（原样） */
    cost_qual_eval(&merge_qual_cost, mergeclauses, root);
    cost_qual_eval(&qp_qual_cost, path->jpath.joinrestrictinfo, root);
    qp_qual_cost.startup -= merge_qual_cost.startup;
    qp_qual_cost.per_tuple -= merge_qual_cost.per_tuple;

    /* 是否可跳过 mark/restore（原样判断） */
    if ((path->jpath.jointype == JOIN_SEMI ||
         path->jpath.jointype == JOIN_ANTI ||
         extra->inner_unique) &&
        (list_length(path->jpath.joinrestrictinfo) ==
    ...
    /* 估算通过 mergequals 的行数（原样：使用 PG 现有统计） */
    mergejointuples = approx_tuple_count(root, &path->jpath, mergeclauses);

    /* 估算需要重取（rescan）的内侧元组数（改 inner_path_rows 为期望） */
    if (IsA(outer_path, UniquePath) || path->skip_mark_restore)
        rescannedtuples = 0;
    else {
        /* CHANGE: 这里原式是 mergejointuples - inner_path_rows，用期望 inner_path_rows */
        rescannedtuples = mergejointuples - inner_path_rows; // CHANGE
        if (rescannedtuples < 0)
            rescannedtuples = 0;
    }

    /* 重取膨胀系数（原样，分母 inner_rows 来自 workspace 的代表/期望） */
    rescanratio = 1.0 + (rescannedtuples / inner_rows);

    /* 不材化：膨胀 inner_run_cost（原样） */
    bare_inner_cost = inner_run_cost * rescanratio;

    /* 材化：重取成本按 cpu_operator_cost 计（原样），但 inner_rows 已是期望/代表 */
    mat_inner_cost = inner_run_cost +
                     cpu_operator_cost * inner_rows * rescanratio;
    ...
    if (path->skip_mark_restore)
        ...
        path->materialize_inner = true;
             ...
             relation_byte_size(inner_path_rows, // CHANGE: 用期望 inner_path_rows
                                inner_path->pathtarget->width) >
             (work_mem * 1024L))
        path->materialize_inner = true;
    else
    ...
    /* CPU 成本（原样使用 workspace 聚合量；这些应由 initial 阶段已妥善设置） */
    startup_cost += merge_qual_cost.startup;
    startup_cost += merge_qual_cost.per_tuple *
            (outer_skip_rows + inner_skip_rows * rescanratio);
    run_cost += merge_qual_cost.per_tuple *
    ...
    /* 输出元组上的额外 quals 成本（原样） */
    startup_cost += qp_qual_cost.startup;
    cpu_per_tuple = cpu_tuple_cost + qp_qual_cost.per_tuple;
    run_cost += cpu_per_tuple * mergejointuples;

    /* tlist 成本（原样；rows 已在上面用分布期望标注） */
    startup_cost += path->jpath.path.pathtarget->cost.startup;
    run_cost += path->jpath.path.pathtarget->cost.per_tuple * path->jpath.path.rows;
...
initial_cost_hashjoin(PlannerInfo *root, JoinCostWorkspace *workspace,
    ...
    Distribution *outer_path_rows_dist = outer_path->rows_dist;
    Distribution *inner_path_rows_dist = inner_path->rows_dist;
...
/* ===================== initial_cost_hashjoin (with distributions) ===================== */
...
initial_cost_hashjoin_exp(
    ...
    /* CHANGE: 线性 CPU 项使用期望值 */
    double E_outer_rows = dist_mean(outer_path_rows_dist, outer_path_rows);
    double E_inner_rows = dist_mean(inner_path_rows_dist, inner_path_rows);

    int num_hashclauses = list_length(hashclauses);

    /* 代表值（供 workspace 使用） */
    int numbuckets_rep = 0; /* CHANGE: 代表性 buckets */
    int numbatches_rep = 1; /* CHANGE: 代表性 batches，默认 1 以保持乐观下界 */
    ...
    /* === 2) 线性 CPU 项：按期望行数计费（独立与否都成立） === */
    // CHANGE: 内侧建表哈希 + 插表：E_inner_rows
    startup_cost += (cpu_operator_cost * num_hashclauses + cpu_tuple_cost) * E_inner_rows;
    // CHANGE: 外侧探测哈希：E_outer_rows
    run_cost += cpu_operator_cost * num_hashclauses * E_outer_rows;

    /* === 3) “是否分批”与 I/O 期望（独立假设下的处理） ===
     * - 判定分批只看内表；因此对 inner 分布逐样本跑 ExecChooseHashTableSize。
     * - 期望 I/O：
     *    startup: seq_page_cost * E[ I * innerpages ]
     *    run:     seq_page_cost * ( E[ I * innerpages ] + 2 * E[ I ] * E[ outerpages ] )
     *   其中 E[I] = q_batch = sum_j p_j * 1{nb_j>1}，
     *   E[outerpages] 对 outer 分布逐样本算。
     */
    double q_batch = 0.0; /* CHANGE: E[I] = P(batching) */
    double E_I_innerpages = 0.0; /* CHANGE: E[I * innerpages] */

    /* CHANGE: 预先计算 E[outerpages]（独立假设下只需外侧边际） */
    double E_outerpages = expect_pages(
        outer_path_rows_dist,
        outer_path->pathtarget->width,
        outer_path_rows
    ); // CHANGE

    /* CHANGE: 针对 inner 分布逐样本判定分批与 innerpages */
    int di_n = inner_path_rows_dist->sample_count;

    /* 代表值：初始取“极乐观” */
    int best_numbatches = INT_MAX;
    int best_numbuckets = 0;

    for (int j = 0; j < di_n; ++j) {
        double pj = inner_path_rows_dist->probs[j];
        double n_i = inner_path_rows_dist->vals[j];

        double n_i_total = n_i;
        if (parallel_hash)
            n_i_total *= get_parallel_divisor(inner_path); /* CHANGE: 样本级反除 */

        size_t space_allowed_dummy; /* unused */
        int numbuckets_j, numbatches_j, num_skew_mcvs_j;

        /* CHANGE: 样本级调用，得到该样本是否需要分批 */
        ...
        if (numbatches_j > 1) {
            double innerpages_j = page_size(n_i, inner_path->pathtarget->width);

            /* 累加：q = E[I]，以及 E[I * innerpages] */
            q_batch += pj;
            E_I_innerpages += pj * innerpages_j;
        }

        /* CHANGE: 选最小的 numbatches 作为代表（维持“下界/乐观”） */
        if (numbatches_j < best_numbatches) {
            best_numbatches = numbatches_j;
            best_numbuckets = numbuckets_j;
        }
    }

    /* 如果 di_n>0，更新代表值；若始终没进入循环则保持默认 */
    Assert(best_numbatches != INT_MAX);
    numbatches_rep = best_numbatches;
    numbuckets_rep = best_numbuckets;

    /* CHANGE: 将 I/O 期望加到成本
     * startup：写 inner 分批文件 → seq_page_cost * E[I * innerpages]
     * run    ：读回 inner + 读写 outer → seq_page_cost * (E[I * innerpages] + 2 * q * E[outerpages])
     */
    if (q_batch > 0.0) {
        startup_cost += seq_page_cost * E_I_innerpages;
        run_cost += seq_page_cost * (E_I_innerpages + 2.0 * q_batch * E_outerpages);
    }

    /* === 4) workspace 输出 === */

    // CHANGE: inner_rows_total 改为“期望内行数（样本独立于 worker 划分）”，并考虑 parallel 反除
    double E_inner_rows_total = dist_mean(inner_path_rows_dist, inner_path_rows);
    if (parallel_hash)
        E_inner_rows_total *= get_parallel_divisor(inner_path);
    ...
    // CHANGE: 写入“代表值”（取最小 numbatches 以维持 initial 的下界语义）
    workspace->numbuckets = numbuckets_rep;
    workspace->numbatches = numbatches_rep;

    workspace->inner_rows_total = E_inner_rows_total;
}
...
final_cost_hashjoin(PlannerInfo *root, HashPath *path,
    ...
    /* Mark the path with the correct row estimate */
    if (path->jpath.path.param_info) {
        elog(LOG, "final_cost_hashjoin::[use_ppi]");
        path->jpath.path.rows = path->jpath.path.param_info->ppi_rows;
    } else {
        elog(LOG, "final_cost_hashjoin::[no_ppi]");
        path->jpath.path.rows = path->jpath.path.parent->rows;
    }
...
final_cost_hashjoin_exp(
    ...
    /* CHANGE: 用“结果行数分布（若有）”的期望来标注 path->rows；否则回退原逻辑 */
    if (path->jpath.path.param_info) {
        elog(LOG, "final_cost_hashjoin::[use_ppi]");
        /* 若 ParamPathInfo 支持分布，则优先取其期望 */
        Distribution *ppi_dist = path->jpath.path.param_info->ppi_rows_dist; // 需要你在结构中可选增加该字段
        double ppi_rows = path->jpath.path.param_info->ppi_rows;
        path->jpath.path.rows = dist_mean(ppi_dist, ppi_rows); // CHANGE
    } else {
        elog(LOG, "final_cost_hashjoin::[no_ppi]");
        /* 若 parent RelOptInfo 也挂有 rows_dist，可用其期望；否则回退 */
        Distribution *parent_dist = path->jpath.path.parent->rows_dist; // 需要可选字段
        double parent_rows = path->jpath.path.parent->rows;
        path->jpath.path.rows = dist_mean(parent_dist, parent_rows); // CHANGE
    }

    /* For partial paths, scale row estimate. (unchanged) */
    if (path->jpath.path.parallel_workers > 0) {
        double parallel_divisor = get_parallel_divisor(&path->jpath.path);
        path->jpath.path.rows =
                clamp_row_est(path->jpath.path.rows / parallel_divisor);
    ...
    /* mark the path with estimated # of batches (unchanged) */
    path->num_batches = numbatches;

    /* store the total number of tuples (sum of partial row estimates) (unchanged) */
    path->inner_rows_total = inner_path_rows_total;

    /* and compute the number of "virtual" buckets in the whole join (unchanged) */
    virtualbuckets = (double) numbuckets * (double) numbatches;

    /*
     * Determine bucketsize fraction and MCV frequency for the inner relation.
     * (unchanged except we already computed inner_path_rows as expectation)
     */
    if (IsA(inner_path, UniquePath)) {
        innerbucketsize = 1.0 / virtualbuckets;
        innermcvfreq = 0.0;
    ...
    /*
     * Disable check for inner MCV memory blow-up:
     * 使用 inner_path_rows 的期望（而非单点）参与判断。
     */
    if (relation_byte_size(clamp_row_est(inner_path_rows * innermcvfreq), // CHANGE
                           inner_path->pathtarget->width) > get_hash_memory_limit())
        startup_cost += disable_cost;

    /* Compute cost of quals (unchanged) */
    cost_qual_eval(&hash_qual_cost, hashclauses, root);
    cost_qual_eval(&qp_qual_cost, path->jpath.joinrestrictinfo, root);
    qp_qual_cost.startup -= hash_qual_cost.startup;
    qp_qual_cost.per_tuple -= hash_qual_cost.per_tuple;
    ...
    if (path->jpath.jointype == JOIN_SEMI ||
        ...
        /* CHANGE: outer_matched_rows 用外侧期望行数参与计算 */
        outer_matched_rows = rint(outer_path_rows * extra->semifactors.outer_match_frac); // CHANGE

        inner_scan_frac = 2.0 / (extra->semifactors.match_count + 1.0);

        startup_cost += hash_qual_cost.startup;

        /* CHANGE: inner 相关项用 inner 期望行数 */
        run_cost += hash_qual_cost.per_tuple * outer_matched_rows *
                clamp_row_est(inner_path_rows * innerbucketsize * inner_scan_frac) * 0.5; // CHANGE

        /* CHANGE: unmatched 也用 inner 期望与 outer 期望 */
        run_cost += hash_qual_cost.per_tuple *
                (outer_path_rows - outer_matched_rows) *
                clamp_row_est(inner_path_rows / virtualbuckets) * 0.05; // CHANGE

        /* Get # of tuples that will pass the basic join (unchanged except expectations used above) */
        if (path->jpath.jointype == JOIN_ANTI)
            hashjointuples = outer_path_rows - outer_matched_rows; // CHANGE: 已用期望
        else
            hashjointuples = outer_matched_rows; // CHANGE: 已用期望
    } else {
        startup_cost += hash_qual_cost.startup;

        /* CHANGE: 普通内连接分支使用期望 outer/inner 行数 */
        run_cost += hash_qual_cost.per_tuple * outer_path_rows *
                clamp_row_est(inner_path_rows * innerbucketsize) * 0.5; // CHANGE

        /* approx_tuple_count 仍按 PG 既有统计（JOIN_INNER 语义） */
        hashjointuples = approx_tuple_count(root, &path->jpath, hashclauses);
    }

    /*
     * For each tuple that gets through the hashjoin proper... (unchanged except rows already expectation)
     */
    startup_cost += qp_qual_cost.startup;
    cpu_per_tuple = cpu_tuple_cost + qp_qual_cost.per_tuple;
    run_cost += cpu_per_tuple * hashjointuples;
...
set_baserel_size_estimates(PlannerInfo *root, RelOptInfo *rel) {
    ...
    /* Should only be applied to base relations */
    Assert(rel->relid > 0);
    double sel = clauselist_selectivity(
        root, rel->baserestrictinfo, 0, JOIN_INNER, NULL
    );
    nrows = rel->tuples * sel;

    elog(LOG, "REL[%d] name: %s; tuples: %.5f; sel: %.5f, rows: %.5f",
         rel->relid, get_baserel_alias(root, rel->relid), rel->tuples, sel, nrows);
    ...
    /* We would like to set the rows selectivity of each relation. */
    calc_baserel_rows_from_real_sel(root, rel);
}
...
set_joinrel_size_estimates(PlannerInfo *root, RelOptInfo *rel,
    ...
    rel->rows = calc_joinrel_size_estimate(root,
                                           ...
                                           sjinfo,
                                           restrictlist);
    Selectivity join_sel = rel->rows / rel->tuples;
    elog(LOG, "set_joinrel_size_estimates::[SEL] join_sel = %.5f", join_sel);
}
