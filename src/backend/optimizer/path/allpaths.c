...
generate_gather_paths(PlannerInfo *root, RelOptInfo *rel, bool override_rows) {
    ...
    /*
     * The output of Gather is always unsorted, but we consider all partial
     * paths, not only the cheapest one.
     */
    ListCell *lc_pp;
    foreach(lc_pp, rel->partial_pathlist) {
        Path *partial_path = lfirst(lc_pp);
        rows = partial_path->rows * partial_path->parallel_workers;
        Path *gather_path = (Path *) create_gather_path(
            root, rel, partial_path, rel->reltarget, NULL, rowsp
        );
