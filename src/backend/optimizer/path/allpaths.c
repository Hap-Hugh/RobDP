...
char *joinrel_path_count_file = NULL;
...
standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels) {
    ...
    StringInfoData rel_path_count_log;
    initStringInfo(&rel_path_count_log);
    ...
    for (lev = 2; lev <= levels_needed; lev++) {
        ...
        foreach(lc, root->join_rel_level[lev]) {
            ...
            count_joinrel_path(root, rel, &rel_path_count_log);
    ...
    write_joinrel_path(&rel_path_count_log, joinrel_path_count_file);
